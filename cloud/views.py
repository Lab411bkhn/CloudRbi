...
from cloud.process.RBI import DM_CAL,CA_CAL
...
def NewProposal(request, componentID):
    try:
        ...
        raise Http404
    return render(request, 'FacilityUI/proposal/proposalNormalNew.html',{'api':Fluid})
def NewTank(request, componentID):
    try:
        ...
        # print(e)
        raise Http404
    return render(request, 'FacilityUI/proposal/proposalTankNew.html', {'isshell':isshell})
def EditProposal(request, proposalID):
    try:
        ...
        damageMachinsm = models.RwDamageMechanism.objects.get(id= proposalID)
        ...
        if request.method == 'POST':
            ...
            refullPOF = models.RwFullPof(id=rwassessment, thinningap1=dm_cal.DF_THINNING_TOTAL_API(0),
                                         ...
                                         thinninggeneralap1=dm_cal.DF_THINNING_TOTAL_API(0) + dm_cal.DF_EXT_TOTAL_API(
                                             0),
                                         thinninggeneralap2=dm_cal.DF_THINNING_TOTAL_API(3) + dm_cal.DF_EXT_TOTAL_API(
                                             3),
                                         thinninggeneralap3=dm_cal.DF_THINNING_TOTAL_API(6) + dm_cal.DF_EXT_TOTAL_API(
                                             6),
                                         totaldfap1=TOTAL_DF_API1, totaldfap2=TOTAL_DF_API2, totaldfap3=TOTAL_DF_API3,
                                         pofap1=pofap1, pofap2=pofap2, pofap3=pofap3, gfftotal=gffTotal,
                                         pofap1category=dm_cal.PoFCategory(TOTAL_DF_API1),
            ...
            if ca_cal.NominalDiametter == 0 or ca_cal.STORED_PRESSURE == 0 or ca_cal.MASS_INVERT == 0 or ca_cal.MASS_COMPONENT == 0 or ca_cal.FLUID is None:
                calv1 = models.RwCaLevel1(id=rwassessment, release_phase=ca_cal.GET_RELEASE_PHASE(),
                                          fact_di=ca_cal.fact_di(),
                                          fact_mit=ca_cal.fact_mit(), fact_ait=ca_cal.fact_ait(), fc_total=100000000,
                                          fcof_category="E")
            else:
                calv1 = models.RwCaLevel1(id=rwassessment, release_phase=ca_cal.GET_RELEASE_PHASE(),
            ...
            for damage in damageList:
                damageMachinsm = models.RwDamageMechanism(id_dm=rwassessment, dmitemid_id=damage['DM_ITEM_ID'],
                                                          ...
                                                          inspduedate=dm_cal.INSP_DUE_DATE(calv1.fc_total, gffTotal,
                                                              ...
                                                              '%Y-%m-%d'))
                damageMachinsm.save()
            refullfc = models.RwFullFcof(id=rwassessment, fcofvalue=calv1.fc_total, fcofcategory=calv1.fcof_category,
                                         envcost=data['EnvironmentCost'],
                                         equipcost=data['EquipmentCost'], prodcost=data['ProductionCost'],
                                         popdens=data['PersonDensity'], injcost=data['InjureCost'])
            refullfc.save()
            # data for chart
            riskList = dm_cal.DF_LIST_16(calv1.fc_total, gffTotal, datafaci.managementfactor, target.risktarget_fc)
            chart = models.RwDataChart(id=rwassessment, riskage1=riskList[1], riskage2=riskList[2],
                                       riskage3=riskList[3],
                                       riskage4=riskList[4], riskage5=riskList[5], riskage6=riskList[6],
                                       riskage7=riskList[7],
                                       riskage8=riskList[8], riskage9=riskList[9], riskage10=riskList[10],
    ...
    return render(request, 'FacilityUI/proposal/proposalNormalEdit.html', {'api':Fluid, 'rwAss':rwassessment, 'rwEq':rwequipment,
                                                                           ...
                                                                           'rwCoat':rwcoat, 'rwMaterial':rwmaterial,})
def demo(request):
    if request.method =='POST':
        sleep(4)
    return render(request, 'FacilityUI/proposal/demo.html')